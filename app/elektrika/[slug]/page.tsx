...
function GeoPage({ slug }: { slug: string }) {
  ...
  const faq = [
    { q: `Сколько стоит электрика в доме ${g.prep}?`, a: `Считаем по точкам: от 1 200 ₽ в газобетоне и каркасе, от 1 800 ₽ в брусе; щит на 380 В от 45 000 ₽, ввод СИП от 18 000 ₽, заземление от 18 000 ₽. Дом 120 м² — от 180 000 ₽ с материалами без светильников и фурнитуры. ${g.distance} от Иркутска — рабочая зона, выезд инженера бесплатный, смета фиксируется в договоре.` },
  ...
  return (
    <>
      <JsonLd data={ldService({ name: `Электромонтаж в частном доме ${g.prep}`, description: `Ввод, щит, разводка под электроотопление, заземление, освещение под ключ ${g.prep}`, path: EP.geo(g.slug), area: g.name, priceFrom: 1200 })} />
      <div className="container-site">

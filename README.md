...
Next.js 14 (App Router) + Tailwind, статическая генерация, формы → Telegram. 301 страница. Домен: `fortes-group.ru`.

## 0. Что изменилось в v27 — раздел «Электрика» стал живым

Новый раздел `/elektrika/` (электромонтаж только в частных домах, коттеджах, банях и дачах): хаб, 17 услуг в 4 кластерах (по типу дома / основные работы / защита и резерв / под ключ и сервис), 26 гео с состоянием сетей по посёлкам, `/elektrika/ceny/`, `/elektrika/kalkulyator/`. 46 новых адресов (визитка `/elektrika/` заменена), всего 301. Своих объектов у раздела нет — показывает 4 объекта отопления с электрокотлами (`objectRefs`).
